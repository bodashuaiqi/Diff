...
class DownsampleNetworkResNet18V1(nn.Module):
    """
    Downsampling using ResNet V1 (只保留到layer3 输出空间分辨率更大)
    """
    def __init__(self):
        super(DownsampleNetworkResNet18V1, self).__init__()
        backbone = resnet50(pretrained=True)
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        # x = self.layer4(x)
        return x  # 输出 [N, 1024, 4, 4]

class DownsampleNetworkResNet18V1_Full(nn.Module): #完整的
    """
    Full ResNet backbone for global branch (includes layer4, outputs 2048 channels)
    """
    def __init__(self):
        super().__init__()
        backbone = resnet50(pretrained=True)
        ...
        self.layer2 = backbone.layer2
        self.layer3 = backbone.layer3
        self.layer4 = backbone.layer4  # Keep layer4 for 2048 channels
...
class GlobalNetwork(AbstractMILUnit):
    ...
    def __init__(self, parameters, parent_module):
        ...
        if "use_v1_global" in parameters and parameters["use_v1_global"]:
            self.downsampling_branch = DownsampleNetworkResNet18V1_Full()
        else:
...
class AttentionModule(AbstractMILUnit):
    ...
    def add_layers(self):
        ...
        self.parent_module.mil_attn_V = nn.Linear(512*2, 128, bias=False)
        self.parent_module.mil_attn_U = nn.Linear(512*2, 128, bias=False)
        self.parent_module.mil_attn_w = nn.Linear(128, 1, bias=False)
        # classifier
        self.parent_module.classifier_linear = nn.Linear(512*2, self.parameters["num_classes"], bias=False)
